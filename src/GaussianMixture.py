...
class gaussian_mixture():
    ...
    style : str
        Path to custom matplotlib style.
    latex : bool
        Makes text in plots formatted for latex usage.
    ...
    def __init__(self, X_low : np.array,
                 ...
                 height_plot : int = 3,
                 dpi : int = 100,
                 style : Union[str, None] = r'src/custom.mplstyle',
                 latex : bool = False) -> None:

        # Style
        if isinstance(style, str):
            self.style_name = style  # str
        else:
    ...
    def plot_confidence_1d(self, average_poisson : float = None,
        ...
        for index, (mean, covariance, weight) in enumerate(zip(self.cluster_means,
            ...
            if plot_int:
                with plt.style.context(self.style_name):
                    plt.plot(x, p_sn[index] * p_n[index] / (p_s + self.eps))
